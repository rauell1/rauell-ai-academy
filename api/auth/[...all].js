...
async function getAuthAndHandler() {
  ...
  const nodeHandler = toNodeHandler(auth2);
  cachedAuth = auth2;
  cachedNodeHandler = nodeHandler;
  return { auth: auth2, nodeHandler };
}
async function handler(req, res) {
  try {
    const { auth: auth2, nodeHandler } = await getAuthAndHandler();
    if (!res || typeof req.json === "function" && !req.headers?.host) {
      return auth2.handler(req);
    }
