...
var DEFAULT_DATABASE_URL, DEFAULT_BETTER_AUTH_SECRET, serverEnvSchema;
var init_env = __esm({
  "src/server/env.ts"() {
    "use strict";
    DEFAULT_DATABASE_URL = "postgresql://neondb_owner:npg_Lsw0XAHbth9u@ep-polished-surf-aynkf6hb-pooler.c-5.us-east-2.aws.neon.tech/neondb?channel_binding=require&sslmode=require";
    DEFAULT_BETTER_AUTH_SECRET = "b8f3a925d48440af6e10f59e47e9fe3d9e3be115dfe03042fc19b1467a3e1ded";
...
var publicationState, accountState, enrolmentState, attemptState, projectSubmissionState, certificateState, blockType, questionType, id, timestamps, users, profiles, accounts, sessions, rateLimits, verifications, roles, permissions, userRoles, rolePermissions, pathways, courses, pathwayCourses, modules, lessons, lessonBlocks, courseInstructors, enrolments, lessonProgress, courseProgress, assessments, questions, answerOptions, assessmentAttempts, learnerAnswers, projects, projectSubmissions, submissionReviews, certificates, certificateVerifications, progressImports, storedFiles, contentVersions, auditLogs, userRelations, courseRelations;
var init_schema = __esm({
  "src/server/schema.ts"() {
    "use strict";
    publicationState = pgEnum("publication_state", [
      "draft",
      "in_review",
      "changes_requested",
      "approved",
    ...
    accountState = pgEnum("account_state", [
      "active",
      "suspended",
      "deletion_requested",
      "deleted"
    ]);
    enrolmentState = pgEnum("enrolment_state", [
      "active",
      "completed",
      "withdrawn",
      "archived"
    ]);
    attemptState = pgEnum("attempt_state", [
      "draft",
      "submitted",
      "pending_review",
      "graded"
    ]);
    projectSubmissionState = pgEnum("project_submission_state", [
      "not_started",
      "draft",
      "submitted",
      "under_review",
    ...
    certificateState = pgEnum("certificate_state", [
      "valid",
      "revoked",
      "expired"
    ]);
    blockType = pgEnum("lesson_block_type", [
      "heading",
      "paragraph",
      "rich_text",
      "image",
    ...
    certificateVerifications = pgTable(
    ...
    courseRelations = relations(courses, ({ many }) => ({
...
var progressInput, importInput, learningApi;
...
async function getRoleKeys(userId) {
  return getDb().select({ key: roles.key }).from(userRoles).innerJoin(roles, eq3(userRoles.roleId, roles.id)).where(eq3(userRoles.userId, userId));
}
var PERMISSIONS, AuthorizationError;
...
var courseInput, moduleInput, lessonInput, blockInput, transitionInput, submitAssessment, projectInput, reviewInput, operationsApi;
...
async function getAppAndListener() {
  ...
  const { Hono: Hono3 } = await import("hono");
  const { learningApi: learningApi2 } = await Promise.resolve().then(() => (init_learning(), learning_exports));
  const { operationsApi: operationsApi2 } = await Promise.resolve().then(() => (init_operations(), operations_exports));
  const { AuthorizationError: AuthorizationError2 } = await Promise.resolve().then(() => (init_permissions(), permissions_exports));
  const { getServerEnv: getServerEnv2 } = await Promise.resolve().then(() => (init_env(), env_exports));
  const app = new Hono3().basePath("/api/v1");
  app.use("*", async (c, next) => {
    if (!["GET", "HEAD", "OPTIONS"].includes(c.req.method)) {
      const origin = c.req.header("origin");
      const allowed = [
        getServerEnv2().APP_ORIGIN,
        "https://learn.rauell.systems",
        "http://localhost:5173",
        "http://localhost:3000"
      ];
  ...
  app.route("/", learningApi2);
  app.route("/", operationsApi2);
